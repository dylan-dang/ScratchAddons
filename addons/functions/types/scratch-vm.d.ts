...
/// <reference path="./sb3.d.ts" />

type EventEmitter<T extends Record<keyof T, any>> = import("events").EventEmitter<T>;
...
declare namespace ScratchVM {
  ...
  type MonitorMap = import("immutable").OrderedMap<
    string,
    import("immutable").Record<{
      ...
      sliderMax: number;
      isDiscrete: boolean;
      x: null | number; // (x: null, y: null) Indicates that the monitor should be auto-positioned
      y: null | number;
      width: number;
      height: number;
      visible: boolean;
  ...
  interface PostedSpriteInfo {
    ...
    direction?: number;
    draggable?: boolean;
    rotationStyle?: ScratchVM.RotationStyle;
    visible?: boolean;
    size?: number;
  }
  ...
  export interface VM extends EventEmitter<ScratchVM.VirtualMachineEventMap> {
    runtime: ScratchVM.Runtime;

    renderer: ScratchVM.IfRenderer<RenderWebGL, undefined>;

    /**
     * @see {ScratchVM.Runtime.attachRenderer}
     */
    attachRenderer(renderer: RenderWebGL): void;

    /**
     * @see {ScratchVM.Runtime.attachAudioEngine}
     */
    attachAudioEngine(audioEngine: AudioEngine): void;
    ...
    /**
     * @see {ScratchVM.Runtime.attachV2BitmapAdapter}
     */
    attachV2BitmapAdapter(bitmapAdapter: ScratchSVGRenderer.BitmapAdapter): void;

    /**
     * @see {ScratchVM.Runtime.attachStorage}
     */
    attachStorage(storage: ScratchStorage): void;

    extensionManager: ScratchVM.ExtensionManager;

    /**
     * @see {ScratchVM.Runtime.start}
     */
    start(): void;

    /**
     * @see {ScratchVM.Runtime.quit}
     */
    quit(): void;

    /**
     * @see {ScratchVM.Runtime.greenFlag}
     */
    greenFlag(): void;
    ...
    deserializeProject(json: object, zip?: import("jszip")): Promise<void>;

    installTargets(
      targets: ScratchVM.Target[],
      extensions: ScratchVM.ImportedExtensionsInfo,
    ...
    /**
     * @see {ScratchVM.Runtime.getEditingTarget}
     */
    editingTarget: ScratchVM.Target | null;
     ...
     */
    setVariableValue(targetId: string, variableId: string, value: ScratchVM.VariableValue): boolean;

    getVariableValue(targetId: string, variableId: string): ScratchVM.VariableValue | null;
    ...
    addCostume(md5ext: string, costume?: ScratchVM.Costume, targetId?: string, version?: 2): Promise<void>;

    addCostumeFromLibrary(md5ext: string, costume: ScratchVM.Costume): Promise<void>;

    addBackdrop(md5ext: string, costume?: ScratchVM.Costume): Promise<void>;

    addSound(sound: ScratchVM.Sound, targetId?: string): Promise<void>;
     ...
     */
    shareBlocksToTarget(blocks: ScratchVM.Block[], targetId: string, fromTargetId?: string): Promise<void>;
     ...
     * @see {ScratchVM.Target.postSpriteInfo}
     */
    postSpriteInfo(spriteInfo: ScratchVM.PostedSpriteInfo): void;

    /**
     * The target that's currently being dragged, if any.
     */
    _dragTarget: ScratchVM.Target | null;

    startDrag(targetId: string): void;

    stopDrag(targetId: string): void;

    postIOData(device: "cloud", data: ScratchVM.CloudData): void;
    postIOData(device: "keyboard", data: ScratchVM.KeyboardData): void;
    postIOData(device: "mouse", data: ScratchVM.MouseData): void;
    postIOData(device: "mouseWheel", data: ScratchVM.MouseWheelData): void;
    postIOData(device: "userData", data: ScratchVM.UserDataData): void;
    postIOData(device: "video", data: ScratchVM.VideoData): void;

    setVideoProvider(videoProvider: ScratchVM.VideoProvider): void;

    setCloudProvider(cloudProvider: ScratchVM.CloudProvider): void;

    /**
     * @see {ScratchVM.Runtime.scanForPeripheral}
     */
    scanForPeripheral(extensionID: string): void;

    /**
     * @see {ScratchVM.Runtime.connectPeripheral}
     */
    connectPeripheral(extensionID: string, peripheralId: number): void;

    /**
     * @see {ScratchVM.Runtime.disconnectPeripheral}
     */
    disconnectPeripheral(extensionID: string): void;

    /**
     * @see {ScratchVM.Runtime.getPeripheralIsConnected}
     */
    getPeripheralIsConnected(extensionID: string): boolean;
  }
}
