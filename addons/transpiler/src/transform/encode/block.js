...
export class RegisteredBlock {
  ...
  isReporter() {
    ...
    return (
      !!json.outputShape ||
      !!json.output ||
      json.extensions?.some((ext) => ["output_boolean", "output_number", "output_string"].includes(ext))
    );
  }
  ...
  clonePrimitive(primitive) {
    ...
    switch (type) {
      case InputType.NoShadow:
      case InputType.SameShadow:
        return /** @type {T} */ ([type, this.cloneRef(primitive[1])]);
      case InputType.DifferentShadow:
        return /** @type {T} */ ([type, this.cloneRef(primitive[1]), this.cloneRef(primitive[2])]);
      default:
